function phi = drlse_edge_for_clump_background(phi_0, g, lambda,mu, alfa, epsilon, timestep, iter, potentialFunction, wTrust)
...
phi_start = double((phi > 0).*(bwdist(phi < 0)- 0.5) - (phi < 0).*(bwdist(phi > 0)- 0.5));
...
%         phi=phi + timestep*(mu*distRegTerm + lambda*edgeTerm + alfa*areaTerm);
    phi=phi + timestep*(mu*distRegTerm + lambda*edgeTerm + alfa*areaTerm) + wTrust * phi_start;
